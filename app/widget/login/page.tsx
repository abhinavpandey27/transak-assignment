/**
 * Widget Login Page
 * Handles email input and OTP verification for the crypto widget
 */
...
export default function LoginPage() {
  ...
  const brandId = undefined; // Default brand for widget pages
  
  const [step, setStep] = useState<"email" | "otp">("email");
  const [email, setEmail] = useState("");
  
  const { login, verifyOTP, isLoading, error, clearError, isAuthenticated } = useAuthStore();
  
  // Load brand configuration
  const brand = loadBrand(brandId || undefined);
        ...
        {brandId && (
          <div className="text-center mt-4">
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Using brand: <span className="font-medium">{brand.name}</span>
            </p>
          </div>
        )}
      </div>
